...
{
	cat <<'EOF'
Usage: scripts/tests/test-in-place-update-docker.sh \
  --init-system openrc|systemd [--firmwares bios,uefi] \
  --ssh-private-key KEY DISK.img

Run the whole-image A/B slot update Gate inside the pinned OrbStack QEMU
runner: boot slot a, stage an update into slot b over SSH, reboot into b,
confirm b, roll back to a and confirm a. The input disk is mounted read-only
and QEMU writes only to a temporary snapshot.
EOF
}
...
for firmware in "${selected_firmwares[@]}"; do
	docker run --rm \
		--network none \
		--env "VOLATOO_SLOT_QEMU_TIMEOUT=${VOLATOO_SLOT_QEMU_TIMEOUT:-360}" \
		...
		"$runner_image" \
		/repo/scripts/qemu-container/test-in-place-update.sh \
		/inputs/disk "$init_system" "$firmware"
done

echo "Volatoo in-place A/B slot update Gate passed ($firmwares)"
